...
@section ('title',  trans('labels.lms.lessons.management') . ' | ' . trans('labels.lms.lessons.deleted'))
...
@section('page-header')
    <h1>
        ...
        <small>{{ trans('labels.lms.lessons.deleted') }} </small>
    </h1>
@endsection
...
@section('content')
    <div class="box box-success">
        <div class="box-header with-border">
            <h3 class="box-title">{{ trans('labels.backend.access.lessons.deleted') }}</h3>
...
//                    {data: 'updated_at', name: '{{config('access.users_table')}}.updated_at'},
            ...
            $("body").on("click", "a[name='delete_lesson_perm']", function(e) {
                e.preventDefault();
                var linkURL = $(this).attr("href");
                swal({
                    title: "{{ trans('strings.backend.general.are_you_sure') }}",
                    text: "{{ trans('strings.lms.lessons.delete_lesson_confirm') }}",
                    type: "warning",
            ...
            $("body").on("click", "a[name='restore_lesson']", function(e) {
                e.preventDefault();
                var linkURL = $(this).attr("href");
                swal({
                    title: "{{ trans('strings.backend.general.are_you_sure') }}",
                    text: "{{ trans('strings.lms.lessons.restore_lesson_confirm') }}",
                    type: "warning",
                    showCancelButton: true,
                    confirmButtonColor: "#DD6B55",
                    confirmButtonText: "{{ trans('strings.backend.general.continue') }}",
