...
@section ('title',  trans('labels.lms.questions.management') . ' | ' . trans('labels.lms.questions.deleted'))
...
@section('page-header')
    <h1>
        ...
        <small>{{ trans('labels.lms.questions.deleted') }} </small>
    </h1>
@endsection
...
@section('content')
    <div class="box box-success">
        <div class="box-header with-border">
            <h3 class="box-title">{{ trans('labels.backend.access.questions.deleted') }}</h3>
...
//                    {data: 'updated_at', name: '{{config('access.users_table')}}.updated_at'},
            ...
            $("body").on("click", "a[name='delete_question_perm']", function(e) {
                e.preventDefault();
                var linkURL = $(this).attr("href");
                swal({
                    title: "{{ trans('strings.backend.general.are_you_sure') }}",
                    text: "{{ trans('strings.lms.questions.delete_question_confirm') }}",
                    type: "warning",
            ...
            $("body").on("click", "a[name='restore_question']", function(e) {
                e.preventDefault();
                var linkURL = $(this).attr("href");
                swal({
                    title: "{{ trans('strings.backend.general.are_you_sure') }}",
                    text: "{{ trans('strings.lms.questions.restore_question_confirm') }}",
                    type: "warning",
                    showCancelButton: true,
                    confirmButtonColor: "#DD6B55",
                    confirmButtonText: "{{ trans('strings.backend.general.continue') }}",
